...
for data_name in dataset_names:
    n_boot=20
    training_data=pickle.load(open("./../modeling/"+data_name+"/Training_data.pkl","rb"))
    testing_data=pickle.load(open("./../modeling/"+data_name+"/Testing_data.pkl","rb"))
    ...
    data = pd.read_csv("./../dataset/"+data_name+".csv", index_col=0)
    ...
    for i in range(n_boot):
        ...
        data_bootstrap=data_inliers[  data_inliers.index.isin(train_idx)]
        data_bootstrap = data_bootstrap.drop(['true_label', 'true_error'], axis=1)  
        predictor = TabularPredictor(**predictorArgs)

...
def score_sig(label: np.array, pred_values: np.array,u_x: np.array,sig_x:np.array) -> np.array:
    ...
    residual = abs(pred_values - label)/sig_x
    scores = np.exp(-(residual))
    return scores
...
for data_name in dataset_names:
    ...
    predictions_mat=np.zeros([len(models_to_be_considered),len(predictions[models_to_be_considered[0]]) ])
    for i in range(len(models_to_be_considered)):
        predictions_mat[i,:]=predictions[models_to_be_considered[i]]
    sigma_ag=np.sqrt(np.var(predictions_mat,axis=0))
    #generate a dictionary of sigma
    ...
    for path in all_u_pred_path:
        ...
        if model_name in models_to_be_considered:
            ...
            u[model_name]=np.sqrt( np.var(bootstrap_result,axis=0) )
...
pickle.dump(all_metrics, open("All_metrics_all.pkl","wb"))
